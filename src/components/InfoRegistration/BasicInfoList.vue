  ...
  >
    ...
    <div style="height: 60%; padding-top: 10%">
      ...
      <div style="display: flex; justify-content: center; align-items: center">
        ...
        />
        <div v-if="num == 1" id="disable-form">{{ myData.age }}</div>
        <span v-if="num == 1">세</span>
        ...
        <div v-if="num == 4" style="display: flex">
          <v-col>
            <v-row
              ><span>1위</span>
              <div id="consume-form">{{ myData.consumePtn1 }}</div></v-row
            >
            <v-row style="margin-top: 30px"
              ><span>2위</span>
              <div id="consume-form">{{ myData.consumePtn2 }}</div></v-row
            >
            <v-row style="margin-top: 30px"
              ><span>3위</span>
              <div id="consume-form">{{ myData.consumePtn3 }}</div></v-row
            >
          </v-col>
        </div>
        <div v-if="num == 5" id="disable-form">
          {{ addComma(myData.assets) }}
        </div>
        <span v-if="num == 5">원</span>
        <div v-if="num == 6" id="disable-form">{{ myData.investProp }}</div>
        <div v-if="num == 7" style="width: 250px" id="disable-form">
          <v-select
            v-model="user.hobby"
            :items="hobbyItems"
            placeholder="취미를 선택하세요."
